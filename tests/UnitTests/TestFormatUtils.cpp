...
TEST(parse_and_validate_tx_extra, is_valid_tx_extra_parsed)
{
  ...
  ASSERT_TRUE(currency.constructMinerTx(0, 0, 10000000000000, 1000, currency.minimumFee(), acc.getAccountKeys().address, tx, b));
...
TEST(parse_and_validate_tx_extra, fails_on_big_extra_nonce)
{
  ...
  ASSERT_FALSE(currency.constructMinerTx(0, 0, 10000000000000, 1000, currency.minimumFee(), acc.getAccountKeys().address, tx, b));
}
